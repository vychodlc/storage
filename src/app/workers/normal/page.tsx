...
export default function NormalWorkersPage() {
  const [data, setData] = useState<Worker[]>([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [editingItem, setEditingItem] = useState<Worker | null>(null);
  const [form] = Form.useForm();

  const normalWorkers = useMemo(() => {
    return mockData.filter((w) => w.type === 'normal');
  }, []);

  React.useEffect(() => {
    setData(normalWorkers);
  }, [normalWorkers]);
  ...
  return (
    <div>
      ...
      <Modal
        title={editingItem ? '编辑普通工人' : '新增普通工人'}
        open={modalVisible}
        onOk={handleSubmit}
        onCancel={() => {
          setModalVisible(false);
      ...
      >
        <Form form={form} layout="vertical">
          ...
          >
            <InputNumber min={0} style={{ width: '100%' }} placeholder="日工资" />
          </Form.Item>
