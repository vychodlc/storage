...
export default function TemporaryWorkersPage() {
  ...
  const temporaryWorkers = useMemo(() => {
    return mockData.filter((w) => w.type === 'temporary');
  }, []);

  React.useEffect(() => {
    setData(temporaryWorkers);
  }, [temporaryWorkers]);
      ...
      render: (_, record) => [
        ...
        <a
          ...
          onClick={() => {
            Modal.confirm({
              title: '确认删除',
              content: `确定要删除临时工 "${record.name}" 吗？`,
              onOk: () => {
